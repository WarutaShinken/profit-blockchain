...
from profit.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from profit.full_node.full_node_api import FullNodeAPI
from profit.server.start_farmer import service_kwargs_for_farmer
from profit.server.start_full_node import service_kwargs_for_full_node
from profit.server.start_harvester import service_kwargs_for_harvester
from profit.server.start_introducer import service_kwargs_for_introducer
from profit.server.start_service import Service
from profit.server.start_timelord import service_kwargs_for_timelord
from profit.server.start_wallet import service_kwargs_for_wallet
from profit.simulator.start_simulator import service_kwargs_for_full_node_simulator
from profit.timelord.timelord_launcher import kill_processes, spawn_process
...
from profit.util.ints import uint16, uint32
from profit.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["profit_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "profit")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["profit_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "profit")
