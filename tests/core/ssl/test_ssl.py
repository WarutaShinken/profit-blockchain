...
from profit.server.outbound_message import NodeType
from profit.server.server import ProfitServer, ssl_context_for_client
from profit.server.ws_connection import WSProfitConnection
from profit.ssl.create_ssl import generate_ca_signed_cert
...
async def establish_connection(server: ProfitServer, dummy_port: int, ssl_context) -> bool:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node):
        ...
        server_1: ProfitServer = full_node_api.full_node.server
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.profit_ca_crt_path.read_bytes(), farmer_server.profit_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.profit_ca_crt_path, farmer_server.profit_ca_key_path, pub_crt, pub_key
        )
        connected = await establish_connection(farmer_server, 12312, ssl_context)
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.profit_ca_crt_path.read_bytes(),
            full_node_server.profit_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.profit_ca_crt_path, full_node_server.profit_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.profit_ca_crt_path.read_bytes(), wallet_server.profit_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.profit_ca_crt_path, wallet_server.profit_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.profit_ca_crt_path.read_bytes(),
            harvester_server.profit_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.profit_ca_crt_path, harvester_server.profit_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.profit_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.profit_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.profit_ca_crt_path.read_bytes(),
            introducer_server.profit_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.profit_ca_crt_path, introducer_server.profit_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.profit_ca_crt_path.read_bytes(),
            timelord_server.profit_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.profit_ca_crt_path, timelord_server.profit_ca_key_path, pub_crt, pub_key
        )
