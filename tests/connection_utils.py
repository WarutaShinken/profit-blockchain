...
async def disconnect_all_and_reconnect(server: ProfitServer, reconnect_to: ProfitServer) -> bool:
...
async def add_dummy_connection(
    server: ProfitServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.profit_ca_crt_path.read_bytes(), server.profit_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.profit_ca_crt_path, server.profit_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: ProfitServer, server_2: ProfitServer) -> WSProfitConnection:
    """
