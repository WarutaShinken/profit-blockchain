...
from profit.cmds.passphrase_funcs import get_current_passphrase
from profit.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from profit.util.keychain import KeyringMaxUnlockAttempts
from profit.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    profit = sys.argv[0]
    process = subprocess.Popen(f"{profit} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
