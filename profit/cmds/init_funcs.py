...
from profit.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_profit_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from profit.util.bech32m import encode_puzzle_hash
from profit.util.config import (
    create_default_profit_config,
    initial_config_file,
    load_config,
    save_config,
...
from profit.util.ssl_check import (
...
from profit.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from profit.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if updated_target:
        print(
            f"To change the PROFIT destination addresses, edit the `gl_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    profit_ca_crt, profit_ca_key = get_profit_ca_crt_key()
    profit_ca_crt_path = ca_dir / "profit_ca.crt"
    profit_ca_key_path = ca_dir / "profit_ca.key"
    write_ssl_cert_and_key(profit_ca_crt_path, profit_ca_crt, profit_ca_key_path, profit_ca_key)
    ...
    profit_ca_crt, profit_ca_key = get_profit_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, profit_ca_crt, profit_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                profit_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return profit_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def profit_version_number() -> Tuple[str, str, str, str]:
...
def profit_minor_release_number():
    res = int(profit_version_number()[2])
    print(f"Install release number: {res}")
    return res


def profit_full_version_str() -> str:
    major, minor, patch, dev = profit_version_number()
