...
ROOT = pathlib.Path(importlib.import_module("profit").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `profit` file name (which is the executable)
    and the `profit` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `profit` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("profit/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("profit-blockchain"))[0]
...
# TODO: collapse all these entry points into one `profit_exec` entrypoint that accepts the server as a parameter

entry_points = ["profit.cmds.profit"] + [f"profit.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "profit.util.bip39"])

if THIS_IS_WINDOWS:
    profit_mod = importlib.import_module("profit")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/profit/util/english.txt", "profit/util"))
datas.append((f"{ROOT}/profit/util/initial-config.yaml", "profit/util"))
datas.append((f"{ROOT}/profit/wallet/puzzles/*.hex", "profit/wallet/puzzles"))
datas.append((f"{ROOT}/profit/ssl/*", "profit/ssl"))
...
add_binary("profit", f"{ROOT}/profit/cmds/profit.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/profit/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/profit/server/start_{server}.py", COLLECT_ARGS)
