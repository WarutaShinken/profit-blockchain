...
from profit.util.default_root import DEFAULT_KEYS_ROOT_PATH
from profit.util.file_keyring import FileKeyring
from profit.util.misc import prompt_yes_no
...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ profit passphrase set # all the cool kids are doing it!"

MASTER_PASSPHRASE_SERVICE_NAME = "Profit Passphrase"
MASTER_PASSPHRASE_USER_NAME = "Profit Passphrase"
...
def check_legacy_keyring_keys_present(keyring: Union[MacKeyring, WinKeyring]) -> bool:
    ...
    from profit.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def _configure_backend(self) -> Union[LegacyKeyring, FileKeyring]:
        from profit.util.keychain import supports_keyring_passphrase
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from profit.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            if response:
                from profit.cmds.passphrase_funcs import prompt_for_new_passphrase
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from profit.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from profit.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
