...
from profit.daemon.server import service_launch_lock_path, singleton
from profit.server.ssl_context import profit_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        profit_ca_crt, profit_ca_key = profit_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = ProfitServer(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for ProfitServer to be closed")
