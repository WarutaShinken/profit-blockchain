...
from profit.protocols import full_node_protocol, introducer_protocol
from profit.protocols.protocol_message_types import ProtocolMessageTypes
from profit.server.address_manager import AddressManager, ExtendedPeerInfo
from profit.server.address_manager_store import AddressManagerStore
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: ProfitServer = server
    ...
    async def on_connect(self, peer: ws.WSProfitConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSProfitConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSProfitConnection):
