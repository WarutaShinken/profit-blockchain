...
from profit.protocols.protocol_state_machine import message_requires_reply
from profit.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from profit.protocols.shared_protocol import protocol_version
from profit.server.introducer_peers import IntroducerPeers
from profit.server.outbound_message import Message, NodeType
from profit.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from profit.util.errors import Err, ProtocolError
from profit.util.ints import uint16
from profit.util.network import is_localhost, is_in_network
from profit.util.ssl_check import verify_ssl_certs_and_keys
...
class ProfitServer:
    def __init__(
        ...
        profit_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSProfitConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSProfitConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.profit_ca_crt_path, self.profit_ca_key_path = profit_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSProfitConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.profit_ca_crt_path, self.profit_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSProfitConnection] = None
        try:
            connection = WSProfitConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSProfitConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.profit_ca_crt_path, self.profit_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSProfitConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSProfitConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSProfitConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSProfitConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSProfitConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSProfitConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSProfitConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSProfitConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use profit's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.profitcrypto.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSProfitConnection, trusted_peers: Dict) -> bool:
