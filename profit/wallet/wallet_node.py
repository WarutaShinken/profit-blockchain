...
from profit.daemon.keychain_proxy import (
    KeychainProxy,
...
from profit.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from profit.protocols import farmer_protocol, wallet_protocol
from profit.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from profit.util.errors import Err, ValidationError
from profit.util.ints import uint32, uint64, uint128
from profit.util.keychain import Keychain
from profit.util.lru_cache import LRUCache
from profit.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from profit.util.path import mkdir, path_from_root
from profit.util.profiler import profile_task
from profit.wallet.block_record import HeaderBlockRecord
from profit.wallet.derivation_record import DerivationRecord
from profit.wallet.settings.settings_objects import BackupInitialized
from profit.wallet.transaction_record import TransactionRecord
from profit.wallet.util.backup_utils import open_backup_file
from profit.wallet.util.wallet_types import WalletType
from profit.wallet.wallet_action import WalletAction
from profit.wallet.wallet_blockchain import ReceiveBlockResult
from profit.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def on_connect(self, peer: WSProfitConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def update_stakings(self, peer: WSProfitConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSProfitConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSProfitConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSProfitConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSProfitConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSProfitConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSProfitConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
