...
from profit.cmds.init_funcs import check_keys, profit_init
from profit.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from profit.daemon.keychain_server import KeychainServer, keychain_commands
from profit.daemon.windows_signal import kill
from profit.plotters.plotters import get_available_plotters
from profit.plotting.util import add_plot_directory
from profit.server.server import ssl_context_for_root, ssl_context_for_server
from profit.ssl.create_ssl import get_mozilla_ca_crt
from profit.util.profit_logging import initialize_logging
from profit.util.config import load_config
from profit.util.json_util import dict_to_json_str
from profit.util.keychain import (
    Keychain,
...
from profit.util.service_groups import validate_service
from profit.util.setproctitle import setproctitle
from profit.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Profit.")
    quit()
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "profit_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in profit_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    profit_init(root_path, should_check_keys=(not wait_for_unlock))
