...
import profitEnvironment from '../util/profitEnvironment';
import profitConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!profitEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    profitConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!profitConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !profitConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (profitConfig.manageDaemonLifetime()) {
        profitEnvironment.startProfitDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
          ...
          },
      {
            ...
            click: () => {
              openExternal(
                'https://www.profitcrypto.net/faq',
              );
            },
          },
      {
          ...
          },
      {
          ...
          {
            label: i18n._(/* i18n */ { id: 'Profit Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/ProfitCrypto/profit-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ProfitCrypto/profit-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ProfitCrypto/profit-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/ProfitCrypto/profit-blockchain/issues',
              );
            },
          },
      {
          ...
          },
      {
          ...
          {
            label: i18n._(/* i18n */ { id: 'Follow Profit on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/ProfitCryptoNet');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Profit Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Profit' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Profit Blockchain' }),
            click: () => {
              openAbout();
            },
          },
