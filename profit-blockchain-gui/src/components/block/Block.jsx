...
export default function Block() {
  ...
  const poolReward = mojo_to_profit(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_profit(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const profitFees = blockRecord.fees
    ? mojo_to_profit(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: profitFees ? `${profitFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Profit blockchain
            </Trans>
          </Back>
        }
        action={
