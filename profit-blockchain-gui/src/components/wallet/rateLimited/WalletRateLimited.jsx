...
import { AlertDialog, Card, Flex } from '@profit/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const profitper_input = ip_parsed.limit;
    ...
    const profitper_value = Number.parseInt(Number(profitper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (profit per interval):{' '}
                  {mojo_to_profit_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = profit_to_mojo(fee_input.value);
