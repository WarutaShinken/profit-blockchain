...
import { AlertDialog } from '@profit/core';
import {
  Typography,
  Button,
  Box,
...
import { profit_to_mojo } from '../../../util/profit';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      profitper_input.value === '' ||
      Number(profitper_input.value) === 0 ||
      !Number(profitper_input.value) ||
      isNaN(Number(profitper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const profitper = profit_to_mojo(profitper_input.value);
    const profitper_value = Number.parseInt(Number(profitper));
    const userpubkey = userpubkey_input.value;
    const amount = profit_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = profit_to_mojo(fee_input.value);
