...
electron-packager . Profit --asar.unpack="**/daemon/**" --platform=darwin \
--icon=src/assets/img/Profit.icns --overwrite --app-bundle-id=net.profit.blockchain \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Profit-darwin-arm64/Profit.app --platform=darwin \
...
mv Profit-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Profit-$PROFIT_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Profit-darwin-arm64/Profit.app Profit-$PROFIT_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.profit.blockchain \
