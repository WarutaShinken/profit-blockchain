...
RPM_NAME="$APP_NAME-$PROFIT_INSTALLER_VERSION"
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	...
	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/ProfitCrypto/profit-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $PROFIT_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
