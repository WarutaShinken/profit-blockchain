...
## 1.2.11 Profit blockchain 2021-11-4

Farmers rejoice: today's release integrates two plotters in broad use in the Profit community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/ProfitCrypto/profit-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/ProfitCrypto/profit-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Profit Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `profit init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `profit plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 Profit blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/ProfitCrypto/profit-blockchain/wiki/Passphrase-Protected-Profit-Keys-and-Key-Storage-Migration). We also launched a new Chialisp compiler in clvm_tools_rs which substantially improves compile time for Chialisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/ProfitCrypto/profit-blockchain/wiki/Passphrase-Protected-Profit-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chialisp compiler in clvm_tools_rs in profit-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other profit services.
...
- Updated the old comment on profit/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Profit DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Profit blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Profit blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Profit's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://profitcrypto.net/download/>.
...
- We have made many full node changes to support our upcoming Profit Asset Token (CAT) standard and our upcoming standalone light wallet, which will use Profit's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming Profit Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install profit-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the profit install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to profit wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Profit's service ip.profitcrypto.net.
...
## 1.2.3 Profit blockchain 2021-07-26
...
- Thanks @mishan for your change to 'profit plotnft show' to display Percent Successful Points.
...
## 1.2.2 Profit blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/ProfitCrypto/profit-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Profit blockchain 2021-07-12
...
## 1.2.0 Profit blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `profit plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/ProfitCrypto/profit-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/ProfitCrypto/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `profit farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `profit configure` now supports command line updates to peer count and target peer count.
...
- All Profit repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `profit plotnft` to manage pools.
...
- Correctly display private key in `profit keys show`.
- Thanks to @sitecurtins for removing a default printout of the private key mnemonic in `profit keys show`.
...
- Thank you to @ProfitMineJP for various improvements.
...
- `profit farm summary` better handles wallet errors.
...
### 1.1.7 Profit Blockchain 2021-06-05

## 1.1.6 Profit Blockchain 2021-06-05
...
## 1.1.6 Profit Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /profit/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `profit configure -t true|false`).
...
## 1.1.5 Profit Blockchain 2021-05-09
...
## 1.1.4 Profit Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `profit keys show`. Use `profit keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Profit Blockchain 2021-05-01
...
## 1.1.2 Profit Blockchain 2021-04-24
...
## 1.1.1 Profit Blockchain 2021-04-21
...
## 1.1.0 Profit Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Profit Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
...
## 1.0.5 Profit Blockchain 2021-04-14
...
## 1.0.4 Profit Blockchain 2021-04-12
...
- The `src` directory in profit-blockchain has been changed to `profit` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `profit/`
...
## 1.0.3 Profit Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/ProfitCrypto/profit-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `profit keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of chiapos 1.0.0 as well.

## 1.0.2 Profit Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [chiapos](https://github.com/ProfitCrypto/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/profit-blockchain).
...
- @jespino changed `profit keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `profit keys show`.
...
## 1.0.1 Profit Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `profit keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/profit-blockchain/).
...
- Users can now pip install e.g. profit-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Profit Blockchain 2021-03-17

### Added

- This is the first production release of the Profit Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable PROFIT. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/profit-blockchain).
...
- Migration with PROFIT_ROOT set does not crash profit init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TPROFIT Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.profitcrypto.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `profit farm summary` will now display TPROFIT or PROFIT as appropriate.
...
- `profit farm summary` displayed some incorrect amounts.
...
- `profit netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `profit plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `profit keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
...
- Generators have been re-worked in CLVM. We added a chialisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during ProfitLisp native deserialization.
...
- We have moved to a new and much higher scalability download.profitcrypto.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.profitcrypto.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.profitcrypto.net/latest/Setup-MacOS.dmg).
- Profit wheels not on Pypi and some dependecies not found there also are now on pypi.profitcrypto.net.
...
- PRs and commits to the profit-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TPROFIT farmed.
- `profit farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `profit wallet` had various bugs.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TPROFIT will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.profitcrypto.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `profit farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/profit-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `profit wallet` now takes get_address to get a new wallet receive address from the CLI.
- `profit plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of profit-blockchain, etc., instead of in blspy.
...
- chiabip158 was set to a profit 1.0 version.
- Comments to Chialisp and clvm source have been updated for all of the Chialisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in profit-blockchain.
...
- Adding Click changed the behavior of `profit keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
...
- All of the profit sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Profit dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `profit -upnp f` was added to disable uPnP.
- `profit plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major chialisp changes. TPROFIT from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/profit-blockchain).
- You can obtain a new wallet receive address on the command line with `profit wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/ProfitCrypto/profit-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `profit -h` and `profit SUBCOMMAND -h` can be your guide.
...
- `profit plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `profit netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `profit init` only migrates release candidate directories. The versioned sub directories under `~/profit` will be going away before mainnet.
...
- `profit configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/ProfitCrypto/profit-blockchain/releases/tag/1.0rc1) was well.
...
- Installers will now be of the pattern ProfitSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'profit wallet get_transactions' in the command line to see your transactions.
- 'profit wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Profit Blockchain GUI](https://crowdin.com/project/profit-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/ProfitCrypto/profit-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/ProfitCrypto/profit-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `profit plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'profit wallet send' now takes in TPROFIT or PROFIT as units instead of mojos.
...
- The Beta 27 chain is a hard fork. All TPROFIT from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `profit wallet -h` to learn more. Also, `profit wallet` now requires a third argument of `show`, therefor you will use `profit wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [profit blockchain gui](https://crowdin.com/project/profit-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `profit init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `profit init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TPROFIT/PROFIT per block for a total of 9216 TPROFIT per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from profit-blockchain, [profit-blockchain-gui](https://github.com/ProfitCrypto/profit-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `profit-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `profit wallet`, was incorrectly displaying TPROFIT balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/ProfitCrypto/profit-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/ProfitCrypto/profit-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/ProfitCrypto/profit-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `profit plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and ProfitLisp code matches the CI compiled code.
- We have removed the '-r' flag from `profit` as it was being overridden in most cases by the `-r` for restart flag to `profit start`. Use `profit --root-path` instead.
- `profit -h` now recommends `profit netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `profit show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `profit configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `profit plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `profit plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `profit show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `profit show` handles sub block 0 better.
...
- `profit plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `profit plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `profit show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `profit configure --enable-upnp`.
...
- `profit configure` now lets you enable or disable uPnP.
...
- `profit netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `profit show` of the GUI use `profit netspace -d 1000` as `profit netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TPROFIT. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TPROFIT compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TPROFIT.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [profitcrypto.net](https://profitcrypto.net/). Among the improvements this gives the Profit blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TPROFIT from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TPROFIT to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TPROFIT to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `profit plots create`. After obtaining the fingerprint from `profit keys show`, try `profit plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TPROFIT you've won so far. Harvester and Full Node connections have moved to Advanced Options.
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Profit application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.profit/VERSION/config/`
...
- `profit show -s` has been refactored to support the new consensus.
- `profit netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.profit/ directory to speed up total testing time.
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run profit-blockchain.
...
- `profit show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/ProfitCrypto/chiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Profit dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/ProfitCrypto/profit-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install profit-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/ProfitCrypto/profit-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/ProfitCrypto/profit-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Profit GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/ProfitCrypto/profit-blockchain/tree/dev/electron-react/src/locales) of the profit-blockchain repository.
- `profit check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/ProfitCrypto/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all profit services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test profit since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve profit-blockprofitn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Profit Network at backup.chia.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Profit backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chia.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test profit are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.profit` directory.
...
- `profit init` now automatically discovers previous releases in each new release.

### Fixed

- `profit show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `profit show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Profit Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Profit UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (profit run farmer-only, wallet-only, timelord-only)
...
- `profit show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `profit show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Profit simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Profit implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test profit between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test profit from the earlier two releases however.
...
- `profit show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Profit)
...
- See wallet balances in command line: `profit show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `profit keys sign` and `profit keys verify` to allow farmers to certify their ownership of keys.
...
- `profit start wallet-server` changed to `profit start wallet`, for consistency.
...
- Removed legacy scripts such as profit-stop-server, profit-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/ProfitCrypto/bls-signatures) to use the
...
- The profit executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./profit -h` from
`~\AppData\Local\Profit-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Profit.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed PROFIT units to TPROFIT units for testnet.
...
"profit plots create" and "profit plots check".
- Add plot directories to config.yaml from the cli with "profit plots add".
...
- The Windows installer is now signed by a Profit Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.profit/test-plots/`
- We have completely refactored all networking code towards making each Profit service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make profit start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Profit Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting profit now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'profit keys' command line.
...
- We added a `profit keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the profit-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `profit netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `profit-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `profit-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `profit-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `profit-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `profit init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `profit start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `profit-start-harvester`.
...
- The user interface now refers to profit coins with their correct currency code of PROFIT.
...
- There was a regression in Profit Proof of Space ([chiapos](https://github.com/ProfitCrypto/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `profit init` then sync on first start will happen automatically. "\$PROFIT_ROOT" users will need to delete `$PROFIT_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `profit netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during profit init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `profit show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `profit show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/ProfitCrypto/profit-blockchain/wiki) and placed in [INSTALL](https://github.com/ProfitCrypto/profit-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/ProfitCrypto/profit-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of profit action commands. You can run `profit version` or `profit start node` for example. Just running `profit` will show you more options. However `profit-create-plots` continues to use the hyphenated form. Also it's now `profit generate keys` as another example.
- Profit start commands like `profit start farmer` and `profit stop node` now keep track of process IDs in a run/ directory in your configuration directory. `profit stop` is unlikely to work on Windows native for now. If `profit start -r node` doesn't work you can force the run/ directory to be reset with `profit start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/ProfitCrypto/profit-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `profit start wallet-gui` has been chaned to `profit start wallet` which launches but the GUI and server on MacOS and Linux. `profit start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/ProfitCrypto/profit-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `profit stop all` command sometimes fails, use `profit-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `profit init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `profit-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/ProfitCrypto/profit-blockchain/wiki/Updating-beta-software). If you've set `$PROFIT_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `profit show`.

### Changed

- `profit-restart-harvester` has been renamed from `profit-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.profit/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Profit blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/ProfitCrypto/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the profit management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `profit-start-farmer &`. The [README.md](https://github.com/ProfitCrypto/profit-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.profit/release/config, plots/ db/, wallet/ etc. This can be overridden by `export PROFIT_ROOT=~/.profit` for example which would then put the plots directory in `HomeDirectory/.profit/plots`.
- The libraries profit-pos, profit-fast-vdf, and profit-bip-158 have been moved to their own repositories: [chiapos](https://github.com/ProfitCrypto/chiapos), [chiavdf](https://github.com/ProfitCrypto/chiavdf), and [chaibip158](https://github.com/ProfitCrypto/chiabip158). They are brought in by profit-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/ProfitCrypto/bls-signatures).
...
- `profit-regenerate-keys` has been renamed `profit-generate-keys`.
...
- Profit fullnode, farmer and harvester now default to logging to profit.log in the profit-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Profit blockchain.
...
- Added color logging
- Now profit_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to profit_full node, profit_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new profit services were started.
...
- This is the first release of the Profit testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.profitcrypto.net/developer/](https://www.profitcrypto.net/developer/)

[unreleased]: https://github.com/ProfitCrypto/profit-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/ProfitCrypto/profit-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/ProfitCrypto/profit-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/ProfitCrypto/profit-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/ProfitCrypto/profit-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/ProfitCrypto/profit-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/ProfitCrypto/profit-blockchain/releases/tag/Alpha-1.0
